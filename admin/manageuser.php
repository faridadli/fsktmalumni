                    ...
                    <div class="container">
                        <div class="nav-page-area">
                            ...
                            <div class="sidebar">
                                <ul>
                                    <li class="active">
                                        <a href="#" class="active"><i class="fa fa-user"></i>Not Verified</a>
                                    </li>
                                    <li>
                            ...
                            <div class="userrequest">
                                <table id="table" class="overflow-y">
                                    ...
                                    <tbody>
										...
										<?php
											$stmt = $con->prepare("SELECT `id`, `username` , `email`, `token`, `date_email` FROM `alumni` WHERE verified=1 ORDER BY date_email ASC");
											$stmt->execute();
											$result = $stmt->get_result();
											while ($row = $result->fetch_array(MYSQLI_ASSOC)){
												echo "<tr><td>{$row['email']}</td>";
												echo "<td>{$row['username']}</td>";
												echo "<td>{$row['date_email']}</td>";
												echo "<td>
												<form action=\"../mailer/sendmail.php\" method=\"post\">
												<input type=\"username\" value=\"{$row['username']}\" name=\"username\" id=\"username\" hidden>
												<input type=\"email\" value=\"{$row['email']}\" name=\"email\" id=\"email\" hidden>
												<input type=\"token\" value=\"{$row['token']}\" name=\"token\" id=\"token\" hidden>
												<!-- Modal for warning admin from any action event -->
												<div class=\"modal fade\" id=\"alertmessage{$row['id']}\" tabindex=\"-1\" role=\"dialog\" aria-labelledby=\"alertmessage\" aria-hidden=\"true\">
														...
														<div class=\"modal-content\">
															...
															<div class=\"modal-body\">
																<div class=\"px-3 pt-3 text-center\">
																	...
																	<br />
																	<br />
																	<h5>Are you sure to remind user?</h5>
												...
												<button class=\"btn btn-primary\" data-toggle=\"modal\" data-target=\"#alertmessage{$row['id']}\">Remind</button></td></tr>";}
										?>
                                    </tbody>
                                </table>
                            </div>
                                    ...
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            <div class="px-3 pt-3 text-center">
                                                ...
                                                <h4 class="pt-4">The action has been done for this user.</h4>
                                            </div>
                                        </div>
                                    </div>
                                </div>
