...
<body>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $check = $con->prepare("SELECT `password` FROM `alumni` WHERE id=$id");
        $check->execute();
        ...
        if ($valid) {
            ...
            if ($update_password = $con->prepare("UPDATE `alumni` SET `password`=? WHERE id=$id")) {
                $update_password->bind_param('s', $newhash);
