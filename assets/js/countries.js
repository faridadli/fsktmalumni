...
s_a[141] = "Badakhshan|Badghis|Baghlan|Balkh|Bamian|Farah|Faryab|Ghazni|Ghowr|Helmand|Herat|Jowzjan|Kabol|Kandahar|Kapisa|Konar|Kondoz|Laghman|Lowgar|Nangarhar|Nimruz|Oruzgan|Paktia|Paktika|Parvan|Samangan|Sar-e Pol|Takhar|Vardak|Zabol";
...
function populateCountriesEdit(countryElementId, stateElementId, country, state) {
    ...
    countryElement.options[0] = new Option('Select Country', '');
	$('option[value=""]').attr("disabled", "disabled");
    //countryElement.selectedValue = 0;
    for (var i = 0; i < country_arr.length; i++) {
		if(country_arr[i]==country){
			countryElement.options[countryElement.length]= new Option(country_arr[i], country_arr[i], false, true);
		}else{
        	countryElement.options[countryElement.length] = new Option(country_arr[i], country_arr[i]);}
    }
    ...
    // Assigned all countries. Now assign event listener for the states.
    if (stateElementId) {
		populateStatesEdit(countryElementId, stateElementId, country, state);
        countryElement.onchange = function() {
            populateStatesEdit(countryElementId, stateElementId, country, state);
        };
    }
...
function populateStatesEdit(countryElementId, stateElementId, country, state) {
    ...
    for (var i = 0; i < state_arr.length; i++) {
		if(state_arr[i]==state){
			stateElement.options[stateElement.length] = new Option(state_arr[i], state_arr[i], false, true);
		}else{
        	stateElement.options[stateElement.length] = new Option(state_arr[i], state_arr[i]);
		}
    }
